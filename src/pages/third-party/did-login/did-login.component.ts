...
export class DidLoginComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.message = this.GetQueryString("message")|| this.getNavParams().get("message");	
  ...
  onItem(itemId){
	...
	cordova.plugins.appmanager.StartApp( backurl + "?type=did_login&message="+ this.message +"&didNum=" + this.didNum 
	     +"&sign=" + this.sign + "&didPubkey=" + this.didPubkey, 
	      function (data) { }, 
	      function (error) { });
	   });
